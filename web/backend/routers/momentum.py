    ...
    async with service_conn() as conn:
        run_id = await conn.fetchval(
            ...
            INSERT INTO backtest_runs (
                asset_type, universe, lookback_days, top_n, years, horizon_days,
            ...
            params["horizon_days"], params["slippage_bps"], params["commission_bps"],
            params["borrow_cost_bps_annual"], params["risk_free_rate_annual"], json.dumps(result),
        )
    return run_id
...
async def momentum_backtest(
    ...
    trailing-return ranking behind /top-performers, over `years` years,
    holding each pick for `horizon_days` before re-ranking — see
    ...
    if horizon_days not in WINDOWS:
        raise HTTPException(422, f"horizon_days must be one of {sorted(WINDOWS)}")
    if asset_type == "Stock":
        if universe not in STOCK_UNIVERSES:
            raise HTTPException(422, f"universe must be one of {sorted(STOCK_UNIVERSES.keys())}")
    elif asset_type == "Fund":
