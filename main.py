...
input_prompt = st.sidebar.text_input(
    label="Modify the default prompt for analysis (Optional)",
    placeholder="Enter your custom prompt here..."
)

# Wrapper function to integrate user input
def research_tool_wrapper(query: str) -> str:
    return research(company_stock=query, user_prompt=input_prompt)

# Define the research tool
research_tool = Tool(
    name="Research",
    func=research_tool_wrapper,
...
if analyze_button:
    ...
    try:
        if analysis_type == "Basic Info":
            response = basic_stock_info_agent.run(f"Provide the basic Detail Analysis {query}.",callbacks=[st_cb])
            st.success("Basic Analysis Complete")
        elif analysis_type == "Technical Analysis":
             response = technical_analysis_agent.run(f"Provide the Technical Analysis {query}.",callbacks=[st_cb])
             st.success( " Technical Analysis Complete")
        elif analysis_type=="Recommend":
             response = recommend_agent.run(f"Provide an investment recommendation based on all Analysis {query}.",callbacks=[st_cb])
             st.success( " Recomand Analysis Complete")
        elif analysis_type=="Filings Analysis":
             response = filings_analysis_agent.run(f"Analyze the latest filings for {query}.",callbacks=[st_cb])
