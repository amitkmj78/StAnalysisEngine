...
def _build_prompt(ticker: str, context: Dict[str, Any], sentiment_text: str) -> str:
    lines = [
        ...
        f"following forecast for {ticker}. Write a short (3-5 sentence) narrative summary for a "
        f"retail investor. State the model's view, then weigh it against the news/sentiment context "
        f"below, and explicitly say whether they agree or conflict. Do not invent numbers not given "
        f"here. Do not give financial advice beyond describing what the data shows.",
        "",
    ...
    lines.append("")
    lines.append(f"News & sentiment context:\n{sentiment_text}")
    return "\n".join(lines)
